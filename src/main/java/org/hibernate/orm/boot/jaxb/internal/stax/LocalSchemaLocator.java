 ...
 */
package org.hibernate.orm.boot.jaxb.internal.stax;
...
public class LocalSchemaLocator {
	...
	public static URL resolveLocalSchemaUrl(String schemaResourceName) {
		...
		if ( url == null ) {
			throw new org.hibernate.boot.jaxb.internal.stax.XmlInfrastructureException( "Unable to locate schema [" + schemaResourceName + "] via classpath" );
		}
		return url;
	}
	...
	public static Schema resolveLocalSchema(URL schemaUrl) {
		try {
			...
			try {
				...
				throw new org.hibernate.boot.jaxb.internal.stax.XmlInfrastructureException( "Unable to load schema [" + schemaUrl.toExternalForm() + "]", e );
			}
			finally {
				try {
					schemaStream.close();
